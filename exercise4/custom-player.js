...
/* Experimented with this, initially didn't work until I renamed it 'loop0' so it didn't collide with 'loop' defined in a function below  */
let loop0 = document.getElementsByClassName("loop")[0];
...
/* I want the button called 'loop' in the HTML to turn on or off a loop feature on the media playback */
/* I've defined the eventlistener here to corrospond with the HTML button and JS above so that when 'clicked', the media will loop */
loop0.addEventListener("click", loop);

function loop(){
  if(media.loop === true){
    media.loop = false; 
  }
  else {media.loop = true;}
  /* This console log is just to confirm if the button actually connects and whether the function is on or not */
  console.log("Loop on")
}
